...
def movemedia(old_file:str, new_file:str, armed:bool) -> None:
    """Moves all files with basename {old_file} and arbitrary extensions
    to {new_file} retaining the extensions. {old_file} must not contain an
    extension.
    Does not overwrite existing files. Removes the old directory if empty after
    operation, forcefully removes dot-files from the old directory to do so."""
            ...
            for file in glob.glob(glob.escape(old_file)+'.*'):
                ...
                try:
                    ...
                    try:
                        ...
                        # prereq: remove dot files in dir
